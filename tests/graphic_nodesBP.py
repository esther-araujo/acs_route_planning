...
def extract_num_nodes_from_file(filename):
    with open(filename, 'r') as file:
        lines = file.readlines()
        for line in lines:
            if line.startswith("nNodesBP:"):
...
distances_1 = [extract_num_nodes_from_file(filename) for filename in file_names_1]
distances_2 = [extract_num_nodes_from_file(filename) for filename in file_names_2]
